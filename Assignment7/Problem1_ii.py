...
def logistic_regression_gradient_descent(features, labels, learning_rate=0.01, max_iterations=10000):
    ...
    for _ in range(max_iterations):
        ...
        predictions = sigmoid(np.dot(augmented_features, weights))
        
        # Gradient of the loss function
        gradient = np.dot(augmented_features.T, (predictions - labels)) / len(labels)
        
        # Update the weights using gradient descent
        weights -= learning_rate * gradient
        
        # Optionally, we could track the loss over iterations (not done here for simplicity)
...
def train_and_plot_logistic_regression(train_features, train_labels, test_features, test_labels):
    ...
    weights = logistic_regression_gradient_descent(train_features, train_labels)
...
# Train the model and plot the decision boundary
train_and_plot_logistic_regression(train_features, train_labels, test_features, test_labels)

# PART E: Third-order feature transformation and logistic regression
...
# Train the logistic regression model on third-order transformed data
transformed_weights = logistic_regression_gradient_descent(transformed_train_features, transformed_train_labels)
...
print("For 3rd-order transformation, E_in: {}, E_test: {}".format(1 - train_accuracy, 1 - test_accuracy))
